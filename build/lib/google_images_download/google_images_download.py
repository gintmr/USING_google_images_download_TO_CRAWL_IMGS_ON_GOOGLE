...
class googleimagesdownload:
    ...
    def download_image(self,image_url,image_format,main_directory,dir_name,count,print_urls,socket_timeout,prefix,print_size,no_numbering,no_download,save_source,img_src,silent_mode,thumbnail_only,format,ignore_urls):
        ...
        try:
            ...
            try:
                ...
                if image_format == 'DS1iW':
                    image_name = re.split(r"[^\w\s]", image_name)
                    # get str lengths
                    lengths = [len(s) for s in image_name]					 
                    # get index of the longest string
                    longest_ind = lengths.index(max(lengths))					 
                    # get longest str
                    image_name = image_name[longest_ind] + ".jpg"
    ...
    # Create define dict from str
    def _get_dict_from_str(self,s):
	# Using same dict format to simplify downstream processing
        dictionary ={
                "ity": "",
                "oh": "",
                "ow": "",
        ...
        src = re.search('src="(.*?)"|$',s).group()
        src = re.search('"(.*?)"|$',src).group()
        img_class = re.search('img class="(.*?)"|$',s).group()
        img_class = re.search('"(.*?)"|$',img_class).group()
        alt = re.search('alt="(.*?)"|$',s).group()
        alt = re.search('"(.*?)"|$',alt).group()
        dictionary['ou'] = src.replace('"','')
        dictionary['ity'] = img_class.replace('"','')
        dictionary['rh'] = alt.replace('"','')
        return dictionary
    # Finding 'Next Image' from the given raw page
    ...
    # Finding 'Next Image' from the given new raw page
    def _get_next_item_new(self,s):
        start_line = s.find('<img class=')
        if start_line == -1:  # If no links are found then give an error!
            end_quote = 0
            link = "no_links"
            return link, end_quote
        else:
            end_object = s.find('/>', start_line + 1)
            object_raw = str(s[start_line:end_object])
            #remove escape characters based on python version
            version = (3, 0)
            cur_version = sys.version_info
            if cur_version >= version: #python3
                try:
                    object_decode = bytes(object_raw, "utf-8").decode("unicode_escape")
                    final_object = self._get_dict_from_str(object_decode)
                except:
                    final_object = ""
            else:  #python2
                try:
                    final_object = (self._get_dict_from_str(self.repair(object_raw)))
                except:
                    final_object = ""
            return final_object, end_object
    # Getting all links with the help of '_images_get_next_image'
    def _get_all_items(self,page,main_directory,dir_name,limit,arguments):
        ...
        count = 1
        while count < limit+1:
            object, end_content = self._get_next_item_new(page)
            if object == "no_links":
                break
            elif object['rh']=='Google':
                page = page[end_content:]
                continue
            elif object == "":
                page = page[end_content:]
